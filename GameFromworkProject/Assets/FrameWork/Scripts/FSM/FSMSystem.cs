...
public class FSMSystem
{
    ...
    public void AddState(FSMState state)
    {
        if (state == null)
        {
            Debug.LogError("The state you wangt to add is null.");
            return;
        }
        if (states.ContainsKey(state.ID))
        {
            Debug.LogError("The state " + state.ID + " you wangt to add has already been added");
            return;
        }
        state.fsm = this;
    ...
    public void DeleteState(FSMState state)
    {
        if (state == null)
        {
            Debug.LogError("The state you wangt to delete is null.");
            return;
        }
        if (states.ContainsKey(state.ID) == false)
        {
            Debug.LogError("The state " + state.ID + " you want to delete id not exit.");
            return;
        }
